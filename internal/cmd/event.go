...
func (c *EventClient) ListEventsByUser(ctx context.Context, user string, opts *EventOptions) ([]*github.Event, error) {
...
LOOP:
	for {
		events, resp, err := c.Activity.ListEventsPerformedByUser(ctx, user, true, &listOpts)
		if err != nil {
			return nil, err
		}
